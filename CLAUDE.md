...
**An override on the PARENT says nothing about the child (v2.9.2 landmine).** `postcss` was pinned and current at 8.5.26, yet its own dependency `nanoid` sat at 3.3.17 with a high advisory (GHSA-2v37-7h3g-55p8) — the block had no `nanoid` entry at all. Pinning a package floors that package only; every transitive underneath it still floats on whatever the lockfile already holds. When an advisory names a package you do *not* override, add a bounded entry for it rather than assuming a nearby parent pin covers it.
