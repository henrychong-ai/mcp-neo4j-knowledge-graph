...
**A transitive with NO override is stuck too — the fix is to ADD a bounded override, not to reinstall (v2.8.3).** `nanoid` sat at 3.3.17 (`. > vite > postcss > nanoid`) with a high advisory open, even though `postcss@8.5.26` declares `nanoid: ^3.3.17` and 3.3.18 was published. pnpm keeps whatever the lockfile already holds and only re-resolves what it must, so a plain `pnpm install` never floats it and `pnpm audit` stays red. Same mechanism as the stale-floor rule above, one step earlier: an unoverridden transitive is pinned by the lockfile exactly as firmly as an overridden one. Add `nanoid: ">=3.3.18 <4"` (ceiling inside the parent's declared major) rather than waiting for a reinstall to fix it.

**Dependabot cannot bump a package that an override clamps, and its PR looks green anyway (v2.8.3).** Dependabot reads `dependencies`/`devDependencies` and never touches `pnpm.overrides`. PR #79 raises the direct `vite` specifier to `^8.2.1` while `vite: ">=7.3.6 <8"` stays in the override block — resolution snaps straight back to 7.x and the bump is a silent no-op behind a clean-looking diff. This is the inbound counterpart to the v2.8.2 desync landmine: for any package that is simultaneously a direct dependency and an override target (currently only `vite`, declared direct so its override binds at all), the specifier and the override ceiling must move together, by hand, in one commit.
